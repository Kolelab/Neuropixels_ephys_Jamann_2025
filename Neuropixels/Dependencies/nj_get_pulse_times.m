function pulse_times_all_recordings = nj_get_pulse_times(record,sAP)
%getpulsetimes. Generates a cell array of pulse times (stumuli) of all recordings (opto or whisker (single), or the combination (dual))
%
% [sAP,pulse_times_all_recordings] = getpulsetimes(rec_name)
%
%    rec_name: looks for a file with that name to open (sAP) in user
%              defined directory, then detects all opto recordings
%              ('RunOptoStim'), loops through all recordings and generates
%              a list of all stimulus ON times based on the info of the
%              stimuli (at which frequencies + durations pulses were
%              delivered), saves them in a cell array
%              "pulse_times_all_recordings" (output of function)
%
% This function extracts the onsets of stimulations (opto, whisker or both)
% for one sAP file (you have to modify the file location in the script) and
% outputs it into a cell array, with one cell containing all stimuli for
% one protocol.
%
% 2025, Alexander Heimel, based on getpulsetimes by Nora Jamann
