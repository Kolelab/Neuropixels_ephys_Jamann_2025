...
fileList = dir(fullfile('shortrecs','Ctrl','*.mat'));
...
for file_idx = 3 % AH: 1:num_files
    disp(['Parsing file ' file_info.name ', ' num2str(file_idx) ' of ' num2str(num_files)]);

    file_info = fileList(file_idx);
    load(fullfile(file_info.folder,file_info.name))

    try
        SampRateImec = sAP.cellBlock{1, 1}.SampRateIM;
    catch
        SampRateImec = [30000.1167360000];
    end
    ...
    %find bursting cells
    cutoff = 10; %define cutoff for number of bursts fired to be counted as bursting!!
    vec_Instfreq_spont_bursts_bursting = vec_Instfreq_spont_bursts(num_spont_bursts>cutoff); %if at least one burst fired
    ...
    % AH: frequenciesperarea is missing
    ...
    filenametwo = strcat(condition,'_bursting_per_animal_min10');
    sheet1 = file_idx;
    xlRange1 = 'A1';
    xlRange2 = 'B1';
    xlRange3 = 'B2';
    ...
    if ~isempty(area_names_bursting)
        xlswrite(filenametwo,area_names_bursting,sheet1,xlRange2);
    else
        disp('area_names_bursting is empty.')
    end
