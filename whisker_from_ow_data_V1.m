...
addition = 0.1; %indicate the whole window (s) to look for spikes 
addition_all = 0.1; %look for spikes during 100ms window --> both stimuli plus spont spiking!

prompt = 'Whisker first? Yes = 1, No = 0';
toggle_whisker = cell2mat(inputdlg(prompt));
excelsheet_filename = '240901_opto_1st_100mswindow.xlsx';

%% LOAD DATA AND DETECT STIMULUS TIMES

cellname_excel = 'all_cells_num_spikes';
...
%for all opto and whisker stims check if spiking and if yes, how many spikes within a window 
...
for stim = 1:numStim
    ...
    vec_SpikesThisTrial_all = vec_SpikeTimes(vec_SpikeTimes > vec_StimTimes_stim1(stim) & vec_SpikeTimes < (vec_StimTimes_stim1(stim) + addition_all));
    else
